...
public class FindPathToBlock {
    ...
    public static List<BlockPos> findPathToBlock(BlockPos block, int depth, List<BlockPos> routeBlocks, HashSet<BlockPos> alrVisited) {
        ...
        for (int i = 0; i < bestBlockPositions.size(); i++) {
            ...
            // Checks to see if path has been found.

            if (checkIfCanFall(blockInQuestion)) {
                int holeDepth = getDepthOfHole(blockInQuestion);
                if (holeDepth < 4) {
                    if (!alrVisited.contains(blockInQuestion)) {
                        blockToAdd = new BlockPos(blockInQuestion.getX(), blockInQuestion.getY() - holeDepth + 1, blockInQuestion.getZ());
                    }
                }
    ...
    public static int getDepthOfHole(BlockPos blockPos) {
        ...
        while (depth < 50 && y > 0 && ids.mc.theWorld.getBlockState(new BlockPos(blockPos.getX(), y, blockPos.getZ())).getBlock() == Blocks.air) {
            depth++;
            y--;
        }
    ...
    private static boolean checkIfCanWalk(BlockPos blockInQuestion) {
        if ((ids.mc.theWorld.getBlockState(blockInQuestion).getBlock() == Blocks.air
                ...
                && ids.mc.theWorld.getBlockState(new BlockPos(blockInQuestion.getX(), blockInQuestion.getY() + 1, blockInQuestion.getZ())).getBlock() == Blocks.air
                && ids.mc.theWorld.getBlockState(new BlockPos(blockInQuestion.getX(), blockInQuestion.getY() - 1, blockInQuestion.getZ())).getBlock() != Blocks.air) {
            return true;
        }
    ...
    private static boolean checkIfCanFall(BlockPos blockInQuestion) {
        ...
        Block currBlockStateUnder1 = ids.mc.theWorld.getBlockState(new BlockPos(blockInQuestion.getX(), blockInQuestion.getY() - 1, blockInQuestion.getZ())).getBlock();
        Block currBlockStateAbove1 = ids.mc.theWorld.getBlockState(new BlockPos(blockInQuestion.getX(), blockInQuestion.getY() + 1, blockInQuestion.getZ())).getBlock();
        ...
        if (currBlockState == Blocks.air && currBlockStateUnder1 == Blocks.air && currBlockStateAbove1 == Blocks.air && currBlockStateAbove2 == Blocks.air) {
            return true;
        }
